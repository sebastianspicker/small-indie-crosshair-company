...
by `scripts/train-quant-emulator.mjs` into `data/quant-emulator.json` (fingerprint
`1027764490`). The inverse's labels come from the project's own `infer` solver on
5000 deterministic samples across 607 distinct legacy settings. The split is
...
**Measured result:** on held-out solver labels the learned inverse reproduced the
exact solver's **full tuple only about 11.6%** of the time (naive direct assignment:
3.57%). Per-dimension exact rates were length 0.334, thickness 0.589, gap 0.403;
mean absolute errors were length 1.269, thickness 0.667, gap 1.062 px against a
naive combined MAE of 2.610. The forward surrogate reached held-out MAE below one
pixel on all four outputs (length 0.392, width 0.286, near 0.540, far 0.540). The
learned inverse ran in roughly 0.4 µs per call versus a 1.0–1.1 ms p50 for the
exact core (`npm run bench:emulator`), about **2500× faster**.
...
successful outcome; the full account is in chapter 10.
