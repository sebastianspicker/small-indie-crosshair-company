...
- Transcription: `research/corpus/procrosshairs-archive.tsv`; rebuilt JSON
  is verified against the original Git blob in `scripts/corpus.mjs`.
...
produced by `scripts/certify-inverse.mjs` (`npm run certify:inverse`) from the
...
- `research/generated/decision-study.json` (`scripts/decision-study.mjs`,
  `npm run study:decision`) compares the declared `expected`, `worst` and `cvar`
  rules on source-derived targets.
- `research/generated/model-partition.json` (`scripts/model-partition.mjs`,
...
- `research/generated/discriminating-set.json`
  (`scripts/discriminating-set.mjs`, `npm run study:discriminating`) is a greedy
...
`research/generated/ranker-benchmark.json` (`scripts/benchmark-ranker.mjs`,
`npm run bench:ranker`) benchmarks `lib/quant/ranker.js` (learned shortlist plus
exact verification of the declared loss) and `lib/quant/sensitivity.js` (analytic
