...
import { NEW_CVARS } from './cvar-inventory.js';
export const DEFAULT_SETTINGS = Object.freeze({...decodeLegacy('CSGO-aNQn2-upV5w-M8dOd-OOwcf-2pFKO')});
/** Names introduced by the 2000914 update. They are not legacy settings and must not be imported as one. */
const NEW_BUILD_NAMES = new Set(Object.values(NEW_CVARS).map(cvar => cvar.name));
