...
export function solveTarget(settings, options, model, target, legacy, measured = false) {
  ...
  const length = bestInteger(NATIVE_RANGES.length.max, target.length / r, v => (Q(v * r, m.rounding) - target.length) ** 2);
...
export function exactPreimage({ settings, options = {}, model, preserveZero = false }) {
  ...
  const lengthFlags = [];
  for (let v = NATIVE_RANGES.length.min; v <= NATIVE_RANGES.length.max; v++) lengthFlags.push(Q(v * r, m.rounding) === target.length);
  ...
  for (const thickness of thicknesses) {
    ...
    if (!constrainedNearFar) gaps = [[NATIVE_RANGES.gap.min, NATIVE_RANGES.gap.max]];
      ...
      const flags = [];
      for (let g = NATIVE_RANGES.gap.min; g <= NATIVE_RANGES.gap.max; g++) flags.push(Q(g * r, m.rounding) === required);
      gaps = runs(flags);
    }
