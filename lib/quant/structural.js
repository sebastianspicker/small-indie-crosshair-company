 ...
 * docs/engineering/v0.4-conversion-improvement-plan.md §2 and
...
import { NATIVE_RANGES, validateNative } from '../native-settings.js';
import { DUMP_DESCRIPTION_FACTS } from '../cvar-inventory.js';
import { bestInteger } from './inverse.js';
import { height as validHeight } from '../validation.js';
...
/** Nonnegative-input quantizers. Nearest rounds positive ties up, not to even. */
export const Q = (u, rule) => rule === 'nearest' ? Math.floor(u + 0.5) : rule === 'ceil' ? Math.ceil(u) : Math.trunc(u);
...
function predict(native, currentHeight, phi) {
  ...
  const length = Math.max(0, Q(rL * native.length + phi.biasLength, phi.quantLength));
  const minimumBranch = native.thickness === 0 && phi.zeroBranch === 'preserve-literal';
  const width = Math.max(1, Q(rT * native.thickness + phi.biasThickness, phi.quantThickness));
  const u = Q(rG * native.gap + phi.biasGap, phi.quantGap);
...
export function inverseLengthScan(targetLength, r, quant, max = NATIVE_RANGES.length.max) {
  ...
  for (let value = 0; value <= max; value++) {
    const predicted = Q(value * r, quant), error = Math.abs(predicted - targetLength), tie = Math.abs(value - targetLength / r);
...
export function inverseLengthInterval(targetLength, r, quant, max = NATIVE_RANGES.length.max) {
  ...
  for (const k of candidates) {
    ...
    const predicted = Q(value * r, quant);
...
function solveGap(target, width, rG, phi) {
  ...
  const nearAt = v => base + slope * Q(v * rG, phi.quantGap);
...
function invertJoint(target, rT, rG, phi, preserveZero) {
  ...
  for (const value of values) {
    const width = Math.max(1, Q(value * rT, phi.quantThickness));
