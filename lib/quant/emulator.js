...
/** The 19 solver features, in declared order. Validates through scope and legacyGeometry. */
export function emulatorFeatures({ settings, options = {} } = {}) {
  ...
  return [
    ...
    settings.size * ratio, settings.thickness * ratio, settings.gap * ratio,
  ];
}
...
export function trainEmulator(samples, { rounds = 60, learningRate = 0.2, seed = 23092026,
  featureNames = EMULATOR_FEATURE_NAMES, bounds = DEFAULT_BOUNDS } = {}) {
  ...
  for (const name of OUTPUT_ORDER) {
    ...
    outputs[name] = trainStumps(X, y, names.length, rounds, learningRate);
  }
  return { schema: EMULATOR_SCHEMA, version: EMULATOR_VERSION, seed, featureNames: names, learningRate, rounds, bounds: built, outputs };
}

function scoreOutput(output, features, learningRate) {
  let value = output.base;
  for (const stump of output.stumps)
    value += learningRate * (features[stump.feature] <= stump.threshold ? stump.left : stump.right);
  return value;
}
...
function checkStumps(block, names, featureCount, label) {
  ...
  for (const name of names) {
    ...
    for (const stump of output.stumps) {
      if (!stump || typeof stump !== 'object') throw new Error(`${label} ${name} stump must be an object.`);
      if (!Number.isInteger(stump.feature) || stump.feature < 0 || stump.feature >= featureCount)
        throw new Error(`${label} ${name} stump feature is out of range.`);
      if (!isFiniteNumber(stump.threshold) || !isFiniteNumber(stump.left) || !isFiniteNumber(stump.right))
        throw new Error(`${label} ${name} stump numbers must be finite.`);
    }
  }
}
...
function canonOutputs(outputs, names) {
  const out = {};
  for (const name of names) {
    const output = outputs?.[name];
    out[name] = output ? { base: output.base, stumps: (output.stumps || []).map(s => ({ feature: s.feature, threshold: s.threshold, left: s.left, right: s.right })) } : undefined;
  }
  return out;
}
...
function canonicalArtifact(artifact) {
  const out = {
    ...
    outputs: canonOutputs(artifact.outputs, OUTPUT_ORDER),
  };
