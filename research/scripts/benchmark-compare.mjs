...
import { quantile } from '../../lib/solver/statistics.js';
const baseline=process.argv[2];
if(!baseline)throw new Error('Usage: node research/scripts/benchmark-compare.mjs PATH_TO_V0_2_CHECKOUT [--save]');
const current=resolve(new URL('../../',import.meta.url).pathname),roots={baseline:resolve(baseline),current},runs={baseline:[],current:[]};
// Each checkout may use either layout: try the current tree's research/scripts/ path first,
// then fall back to the pre-reorganization scripts/ path (for older baseline checkouts).
const benchmarkScriptIn=root=>['research/scripts/benchmark-quant.mjs','scripts/benchmark-quant.mjs']
 .find(candidate=>existsSync(join(root,candidate))) ?? 'research/scripts/benchmark-quant.mjs';
for(let trial=0;trial<5;trial++)for(const name of (trial%2?['current','baseline']:['baseline','current'])) {
 const result=spawnSync(process.execPath,[benchmarkScriptIn(roots[name])],{cwd:roots[name],encoding:'utf8',timeout:30000});
