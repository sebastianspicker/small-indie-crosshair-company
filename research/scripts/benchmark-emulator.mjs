 ...
 * (see research/scripts/train-quant-emulator.mjs); it is not native evidence and is not wired into
 * the runtime inference path, because its exact-tuple fidelity is too low to replace
 * `infer` (see research/generated/quant-emulator.json and docs/math/10-learned-emulator.md).
 */
...
const report = { schema: 'sicc-emulator-benchmark-v1', runtime: process.version, platform: process.platform, cpu: cpus()[0]?.model,
  ...
  scope: 'Single local Node process, speed only. The learned emulator is research-only and is not on the runtime path. No accuracy or native claim; its exact-tuple fidelity is recorded in research/generated/quant-emulator.json.',
