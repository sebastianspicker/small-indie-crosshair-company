...
} from '../lib/quant/modulation-contract.js';
import { structuralForward, DEFAULT_PHI } from '../lib/quant/structural.js';
import { modulate } from '../lib/quant/modulator.js';
...
test('the feature contract keeps weights null and lists only allowed features', () => {
  const features = JSON.parse(readFileSync(join(root, 'data/modulator-features.json'), 'utf8'));
...
test('inference and the app never import the modulator, emulator or contract', () => {
  const inference = readFileSync(join(root, 'lib/quant/inference.js'), 'utf8');
  assert.doesNotMatch(inference, /modulation-contract/);
  assert.doesNotMatch(inference, /modulator\.js/);
  assert.doesNotMatch(inference, /emulator\.js/);
  for (const file of walk(join(root, 'app'))) {
    const text = readFileSync(file, 'utf8');
    assert.doesNotMatch(text, /from ['"][^'"]*modulator\.js['"]/, `${file} must not import modulator.js`);
    assert.doesNotMatch(text, /from ['"][^'"]*emulator\.js['"]/, `${file} must not import emulator.js`);
    assert.doesNotMatch(text, /\bmodulate\s*\(/, `${file} must not call modulate(`);
  }
});
