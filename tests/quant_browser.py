...
def main():
 ...
 try:
  ...
  with sync_playwright() as p:
   ...
   ready();check('simple mode is the default',page.locator('html').get_attribute('data-mode')=='simple');check('simple view hides the expert controls',page.locator('.quant-simple').is_visible() and not page.locator('.quant-layout').is_visible())
   ...
   page.locator('#q-options-panel summary').click();page.select_option('#q-decision','worst');ready();check('worst-case decision policy selectable','limit largest mismatch' in page.locator('#q-model option').nth(1).inner_text())
