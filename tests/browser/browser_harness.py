...
def mount(page, start_route="quant", crypto_bridge=False):
 ...
 page.set_content(html);page.evaluate('(route)=>{location.hash=route}',start_route);page.add_style_tag(content=(ROOT/'app/styles.css').read_text());page.add_style_tag(content=(ROOT/'app/convert.css').read_text())
 if crypto_bridge:
 ...
 def module(path):
  ...
  if path.name=='client.js':
   ...
   s="""import {infer} from '../../lib/solver/inference.js';import {analyzeScreenshot,analyzeNativeScreenshot} from '../../lib/image/screenshot.js';
export class ResearchWorker{constructor(records){this.records=records;this.ready=Promise.resolve();} async call(type,payload){await new Promise(r=>setTimeout(r,0));return type==='infer'?infer({...payload,records:this.records}):type==='screenshot'?analyzeScreenshot(payload):analyzeNativeScreenshot(payload);} close(){}}"""
  ...
  if path.name=='client.js':
   s=s.replace("new URL('./worker.js',import.meta.url)",json.dumps(module(path.parent/'worker.js')))
