...
for(const path of ['/package.json','/.git/config','/scripts/serve.mjs','/tests/core.test.mjs','/%00','/%5Cetc%5Cpasswd','/%ZZ','/app/../../package.json','/app/%2e%2e/%2e%2e/package.json']) {
...
test('quant worker and research CSV have safe usable MIME types',async()=>{
  const w=await fetch(base+'/app/quant-worker.js');assert.equal(w.status,200);assert.match(w.headers.get('content-type'),/text\/javascript/);assert.match(w.headers.get('content-security-policy'),/worker-src 'self'/);
...
test('HTTP hardened header policy also applies to worker responses',async()=>{
 const response=await fetch(base+'/app/quant-worker.js');assert.equal(response.headers.get('x-frame-options'),'DENY');
