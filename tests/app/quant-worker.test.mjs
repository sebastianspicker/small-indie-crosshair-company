...
test('worker entry initializes, computes, transfers image masks, and returns bounded errors',async()=>{
 const workerURL=new URL('../../app/worker/worker.js',import.meta.url).href;
 ...
 const{raster}=await import('../../lib/geometry/raster.js');const g={length:3,width:2,near:2,far:3},mask=raster(g,{},129),bytes=new Uint8ClampedArray(129*129*4);mask.data.forEach((v,i)=>bytes.set(v?[0,255,0,255]:[0,0,0,255],i*4));const fit=(await call('screenshot',{data:bytes,side:129,seed:[0,255,0]})).result;assert.equal(fit.templateIou,1);assert.ok(fit.mask.data instanceof Uint8Array);assert.deepEqual(fit.geometry,g);
 }finally{await w.terminate();}
});
test('payload-size guard is exact and fail-closed',async()=>{
 const {assertPayloadSize,estimatePayloadBytes,MAX_WORKER_PAYLOAD_BYTES}=await import('../../app/worker/limits.js');
 assert.equal(assertPayloadSize(MAX_WORKER_PAYLOAD_BYTES),MAX_WORKER_PAYLOAD_BYTES);
 assert.throws(()=>assertPayloadSize(MAX_WORKER_PAYLOAD_BYTES+1),/exceeds/);
 for(const bad of [-1,NaN,Infinity,'1',null])assert.throws(()=>assertPayloadSize(bad));
 assert.ok(estimatePayloadBytes({a:'y'.repeat(600)})>1200);
 assert.ok(estimatePayloadBytes('x'.repeat(600000))>MAX_WORKER_PAYLOAD_BYTES);
});
test('worker entry rejects an oversized payload before decoding',async()=>{
 const workerURL=new URL('../../app/worker/worker.js',import.meta.url).href;
 const w=new Worker(`const {parentPort}=require('node:worker_threads');globalThis.self={postMessage:(message,transfer)=>parentPort.postMessage(message,transfer)};import(${JSON.stringify(workerURL)}).then(()=>parentPort.on('message',data=>self.onmessage({data})));`,{eval:true});
 let id=0;const call=(type,payload)=>new Promise((resolve,reject)=>{const n=++id;const timer=setTimeout(()=>reject(new Error('worker timeout')),5000);const listen=r=>{if(r.id===n){clearTimeout(timer);w.off('message',listen);resolve(r);}};w.on('message',listen);w.postMessage({id:n,type,payload});});
 try{assert.match((await call('infer','x'.repeat(600000))).error,/exceeds the 1048576-byte limit/);}
 finally{await w.terminate();}
});
