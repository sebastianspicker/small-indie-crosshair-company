import { $,el,download,copy,fmt } from './dom.js';
import { decodeLegacy,encodeLegacy } from '../lib/sharecode.js';
import { parseLegacyCFG } from '../lib/cfg.js';
import { convert,DEFAULT_MODEL,TARGET_BUILD,predictedGeometry,exportCFG,rgba,validateNative } from '../lib/conversion.js';
...
export function initEditor(presets) {
  ...
  $('import-legacy').addEventListener('click',()=>{
    try {const val=$('legacy-import').value.trim();const next=val.startsWith('CSGO-')?decodeLegacy(val):parseLegacyCFG(val).config;convert(next,opts);rgba(next);settings=next;
