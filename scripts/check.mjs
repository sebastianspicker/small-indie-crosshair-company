...
async function walk(path){for(const e of await readdir(path,{withFileTypes:true})){const f=join(path,e.name);if(e.isDirectory())await walk(f);else if(/\.m?js$/.test(e.name)){
  ...
  const text=await readFile(f,'utf8');if((f.includes('/app/')||f.includes('/lib/'))&&/(?:\.innerHTML\s*=|\beval\s*\(|new Function\s*\()/m.test(text))throw new Error('Unsafe dynamic execution/rendering in '+f);count++;
}}}
