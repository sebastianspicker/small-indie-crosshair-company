...
export async function chapters() { return Promise.all((await readdir(new URL('docs/math/', root))).filter(x => /^0[1-9]-.*\.md$/.test(x)).sort().map(async (file) => ({ file, text: await readFile(new URL('docs/math/' + file, root), 'utf8') }))); }
